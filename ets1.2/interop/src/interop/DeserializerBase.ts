...
import { Tags, CallbackResource, InteropError, InteropExceptionKind } from './SerializerBase';
...
export class DeserializerBase {
    ...
    readBuffer(): ArrayBuffer {
        ...
        return InteropNativeModule._MaterializeBuffer(
            data,
            BigInt(length),
            resource.resourceId,
            resource.hold,
    ...
    readException(): Error {
        const kind = this.readInt8()
        if (kind === InteropExceptionKind.EXCEPTION_INTERFACE) {
            const code = this.readInt32()
            const message = this.readString()
            return new InteropError(code, message)
        } else if (kind === InteropExceptionKind.EXCEPTION_OBJECT) {
            return this.readObject() as Error
        } else {
