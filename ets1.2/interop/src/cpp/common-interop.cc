...
KInteropBuffer impl_MaterializeBuffer(KNativePointer data, KLong length,
                                      KInt resourceId, KNativePointer holdPtr,
...
KOALA_INTEROP_5(MaterializeBuffer, KInteropBuffer, KNativePointer, KLong, KInt,
...
bool setKoalaEtsNapiCallbackDispatcher(EtsEnv *etsEnv, ets_class clazz,
  ...
  ets_method method = etsEnv->GetStaticp_method(clazz, dispatcherMethodName,
                                                dispactherMethodSig);
  if (method == nullptr) {
    return false;
  }
...
bool setKoalaJniCallbackDispatcher(JNIEnv *jniEnv, jclass clazz,
                                   const char *dispatcherMethodName,
                                   const char *dispactherMethodSig) {
  g_koalaJniCallbackDispatcher.clazz = clazz;
  jniEnv->NewGlobalRef(clazz);
  jmethodID method = jniEnv->GetStaticMethodID(clazz, dispatcherMethodName,
                                               dispactherMethodSig);
  if (method == nullptr) {
    return false;
  }
...
KInt impl_ManagedStringWrite(const KStringPtr &string, KSerializerBuffer buffer,
                             KInt bufferSize, KInt offset) {
  interop_memcpy((uint8_t *)buffer + offset, bufferSize, string.c_str(),
                 string.length() + 1);
  return string.length() + 1;
}
KOALA_INTEROP_4(ManagedStringWrite, KInt, KStringPtr, KSerializerBuffer, KInt,
                KInt)

void stringFinalizer(string *ptr) { delete ptr; }
...
int32_t callCallback(KVMContext context, int32_t methodId, uint8_t *argsData,
                     int32_t argsLength) {
#if KOALA_USE_NODE_VM || KOALA_USE_HZ_VM || KOALA_USE_PANDA_VM ||              \
    KOALA_USE_JAVA_VM || KOALA_CJ
...
struct ForeignVMContext {
  ...
  int32_t (*callSync)(KVMContext vmContext, int32_t callback, uint8_t *data,
                      int32_t length);
};
typedef KInt (*LoadVirtualMachine_t)(KInt vmKind, const char *bootFiles,
                                     const char *userFiles,
                                     const char *libraryPath,
                                     const struct ForeignVMContext *foreignVM);
typedef KNativePointer (*StartApplication_t)(const char *appUrl,
                                             const char *appParams,
                                             int32_t loopIterationr);
typedef KBoolean (*RunApplication_t)(const KInt arg0, const KInt arg1);
typedef const char *(*EmitEvent_t)(const KInt type, const KInt target,
                                   const KInt arg0, const KInt arg1);
...
KInt impl_LoadVirtualMachine(KVMContext vmContext, KInt vmKind,
                             const KStringPtr &bootFiles,
                             const KStringPtr &userFiles,
                             const KStringPtr &libraryPath) {
  const char *envClassPath = std::getenv("PANDA_CLASS_PATH");
  if (envClassPath) {
    LOGI("CLASS PATH updated from env var PANDA_CLASS_PATH, %" LOG_PUBLIC "s",
         envClassPath);
  }
  ...
  if (!impl)
    impl = reinterpret_cast<LoadVirtualMachine_t>(
        getImpl(nativeLibPath, "LoadVirtualMachine"));
  if (!impl)
    KOALA_INTEROP_THROW_STRING(vmContext, "Cannot load VM", -1);
  const ForeignVMContext foreignVM = {vmContext, &callCallback};
  return impl(vmKind, bootFilesPath, userFiles.c_str(), nativeLibPath,
              &foreignVM);
}
KOALA_INTEROP_CTX_4(LoadVirtualMachine, KInt, KInt, KStringPtr, KStringPtr,
                    KStringPtr)

KNativePointer impl_StartApplication(const KStringPtr &appUrl,
                                     const KStringPtr &appParams,
                                     KInt loopIterations) {
  static StartApplication_t impl = nullptr;
  if (!impl)
    impl = reinterpret_cast<StartApplication_t>(
        getImpl(nullptr, "StartApplication"));
...
KBoolean impl_RunApplication(const KInt arg0, const KInt arg1) {
  ...
  if (!impl)
    impl =
        reinterpret_cast<RunApplication_t>(getImpl(nullptr, "RunApplication"));
  return impl(arg0, arg1);
}
KOALA_INTEROP_2(RunApplication, KBoolean, KInt, KInt)

KStringPtr impl_EmitEvent(KVMContext vmContext, KInt type, KInt target,
                          KInt arg0, KInt arg1) {
...
#ifdef KOALA_ANI
KStringPtr impl_LoadView(const KStringPtr &className,
                         const KStringPtr &params) {
...
void impl_Free(KNativePointer data) { malloc_finalize(data); }
KOALA_INTEROP_DIRECT_V1(Free, KNativePointer)

KInt impl_ReadByte(KNativePointer data, KLong index, KLong length) {
  if (index >= length)
    INTEROP_FATAL(
        "impl_ReadByte: index %lld is equal or greater than length %lld",
...
void impl_WriteByte(KNativePointer data, KInt index, KLong length, KInt value) {
  if (index >= length)
    INTEROP_FATAL(
        "impl_WriteByte: index %lld is equal or greater than length %lld",
        (long long)index, (long long)length);
  uint8_t *ptr = reinterpret_cast<uint8_t *>(data);
  ptr[index] = value;
}
...
void impl_CopyArray(KNativePointer data, KLong length, KByte *array) {
  if (!array || !data) {
    INTEROP_FATAL(
        "CopyArray called with incorrect nullptr args (array, data):(%p, %p)",
        array, data);
  }
...
void impl_CallCallback(KInt apiKind, KInt callbackKind, KSerializerBuffer args,
                       KInt argsSize) {
...
void setCallbackCallerSync(int apiKind,
                           Callback_Caller_Sync_t callbackCallerSync) {
  CHECK_HAS_NOT_CALLBACK_CALLER(apiKind, g_callbackCallerSync);
  g_callbackCallerSync[apiKind] = callbackCallerSync;
}

void impl_CallCallbackSync(KVMContext vmContext, KInt apiKind,
                           KInt callbackKind, KSerializerBuffer args,
                           KInt argsSize) {
...
void impl_CallCallbackResourceReleaser(KNativePointer releaser,
                                       KInt resourceId) {
  reinterpret_cast<void (*)(KInt)>(releaser)(resourceId);
}
KOALA_INTEROP_V2(CallCallbackResourceReleaser, KNativePointer, KInt)

KInt impl_CallForeignVM(KNativePointer foreignContextRaw, KInt function,
                        KSerializerBuffer data, KInt length) {
  const ForeignVMContext *foreignContext =
      (const ForeignVMContext *)foreignContextRaw;
  // Improve: set actuall callbacks caller/holder/releaser.
  /*
   *(int64_t*)(data + 8) = impl_CallCallbackSync;
   *(int64_t*)(data + 16) = 0;
   *(int64_t*)(data + 24) = 0; */
  return foreignContext->callSync(foreignContext->vmContext, function,
                                  reinterpret_cast<uint8_t *>(data), length);
}
KOALA_INTEROP_4(CallForeignVM, KInt, KNativePointer, KInt, KSerializerBuffer,
                KInt)
...
void resolveDeferred(KVMDeferred *deferred, uint8_t *argsData,
                     int32_t argsLength) {
#ifdef KOALA_NAPI
  auto status =
      napi_call_threadsafe_function((napi_threadsafe_function)deferred->handler,
                                    deferred, napi_tsfn_nonblocking);
  if (status != napi_ok)
    LOGE("cannot call thread-safe function; status=%d", status);
  if (deferred->handler) {
    napi_release_threadsafe_function(
        (napi_threadsafe_function)deferred->handler, napi_tsfn_release);
...
void rejectDeferred(KVMDeferred *deferred, const char *message) {
  ...
  if (deferred->handler) {
    ...
    status =
        env->PromiseResolver_Reject((ani_resolver)deferred->handler, nullptr);
    CHECK_ANI_FATAL(status);
    deferred->handler = nullptr;
  }
#endif
...
#ifdef KOALA_NAPI
void resolveDeferredImpl(napi_env env, napi_value js_callback,
                         KVMDeferred *deferred, void *data) {
...
KVMDeferred *CreateDeferred(KVMContext vmContext,
                            KVMObjectHandle *promiseHandle) {
  ...
  auto status =
      napi_create_promise(env, (napi_deferred *)&deferred->context, &promise);
  if (status != napi_ok)
    LOGE("cannot make a promise; status=%d", status);
  status = napi_create_threadsafe_function(
      env, nullptr, nullptr, resourceName, 0, 1, nullptr, nullptr, deferred,
...
public:
  KoalaWork(InteropVMContext vmContext, InteropNativePointer handle,
...
static void DoQueue(void *handle) { ((KoalaWork *)handle)->Queue(); }
static void DoCancel(void *handle) { ((KoalaWork *)handle)->Cancel(); }

InteropAsyncWork
koalaCreateWork(InteropVMContext vmContext, InteropNativePointer handle,
                void (*execute)(InteropNativePointer handle),
                void (*complete)(InteropNativePointer handle)) {
  return {
...
KoalaWork::KoalaWork(InteropVMContext vmContext, InteropNativePointer handle,
                     void (*execute)(InteropNativePointer handle),
                     void (*complete)(InteropNativePointer handle))
    : vmContext(vmContext), handle(handle), execute(execute),
      complete(complete) {
  napi_env env = (napi_env)vmContext;
  napi_value resourceName = nullptr;
  napi_create_string_utf8(env, "KoalaAsyncOperation", NAPI_AUTO_LENGTH,
                          &resourceName);
  napi_create_async_work(env, nullptr, resourceName, DoExecute, DoComplete,
                         this, (napi_async_work *)&vmWork);
  /* supress unused private fields */
  (void)vmContext;
  (void)vmWork;
}
...
void KoalaWork::Execute() { execute(handle); }
...
KoalaWork::KoalaWork(InteropVMContext vmContext, InteropNativePointer handle,
  ...
  if (g_foreignVMContext == nullptr)
    INTEROP_FATAL("Can not launch async work while foreign VM context is not "
                  "available. Please ensure you have "
...
KStringPtr impl_Utf8ToString(KVMContext vmContext, KNativePointer data,
                             KInt offset, KInt length) {
...
KStringPtr impl_Utf8ToString(KVMContext vmContext, KByte *data, KInt offset,
                             KInt length) {
  KStringPtr result((const char *)(data + offset), length, false);
  return result;
}
...
KOALA_INTEROP_CTX_1(RawUtf8ToString, KStringPtr, KNativePointer)
    ...
    defined(KOALA_ETS_NAPI) || defined(KOALA_ANI) || defined(KOALA_KOTLIN)
KStringPtr impl_StdStringToString(KVMContext vmContext,
                                  KNativePointer stringPtr) {
...
KInteropReturnBuffer impl_RawReturnData(KVMContext vmContext, KInt v1,
                                        KInt v2) {
  void *data = new int8_t[v1];
  interop_memset(data, v1, v2, v1);
  KInteropReturnBuffer buffer = {
      v1, data, [](KNativePointer ptr, KInt) { delete[] (int8_t *)ptr; }};
  return buffer;
}
