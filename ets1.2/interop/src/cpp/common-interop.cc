    ...
    EtsEnv* etsEnv, ets_class clazz, const char* dispatcherMethodName, const char* dispactherMethodSig)
{
    ...
    ets_method method = etsEnv->GetStaticp_method(clazz, dispatcherMethodName, dispactherMethodSig);
    if (method == nullptr) {
        return false;
    }
...
bool setKoalaJniCallbackDispatcher(
    JNIEnv* jniEnv, jclass clazz, const char* dispatcherMethodName, const char* dispactherMethodSig)
{
    g_koalaJniCallbackDispatcher.clazz = clazz;
    jniEnv->NewGlobalRef(clazz);
    jmethodID method = jniEnv->GetStaticMethodID(clazz, dispatcherMethodName, dispactherMethodSig);
    if (method == nullptr) {
        return false;
    }
