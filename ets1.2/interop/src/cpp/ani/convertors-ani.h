 ...
 */
#ifndef CONVERTORS_ANI_H
#define CONVERTORS_ANI_H
...
{
    ani_vm* machines[] = { nullptr };
    ani_size vmCount = 0;
    ANI_GetCreatedVMs(machines, sizeof(machines) / sizeof(ani_vm*), &vmCount);

    if (vmCount == 0 || !machines[0])
        return nullptr;

    ani_env* res = nullptr;
    machines[0]->GetEnv(ANI_VERSION_1, &res);
    return res;
}
