...
struct InteropTypeConverter {
    using InteropType = T;
    static T convertFrom(napi_env env, InteropType value)
    {
        return value;
    }
    static InteropType convertTo(napi_env env, T value)
    {
        return value;
    }
        ...
        if (isArrayBuffer) {
            ...
            if (isDataView) {
                napi_get_dataview_info(
                    env, value, reinterpret_cast<size_t*>(&result.length), &result.data, nullptr, nullptr);
            }
        }
        return result;
    }
...
const KInt UINT8_ARRAY_SIZE = 1;
const KInt UINT16_ARRAY_SIZE = 2;
const KInt UINT32_ARRAY_SIZE = 4;
const KInt BIGUINT64_ARRAY_SIZE = 8;

inline napi_typedarray_type getNapiType(KInt size)
{
    switch (size) {
        case UINT8_ARRAY_SIZE:
            return napi_uint8_array;
        case UINT16_ARRAY_SIZE:
            return napi_uint16_array;
        case UINT32_ARRAY_SIZE:
            return napi_uint32_array;
        case BIGUINT64_ARRAY_SIZE:
...
struct InteropTypeConverter<KInteropReturnBuffer> {
    ...
    static InteropType convertTo(napi_env env, KInteropReturnBuffer value)
    {
        ...
        napi_create_external_arraybuffer(
            env, value.data, value.length * value.elementSize, disposer, clone, &arrayBuffer);
...
ElemType* getTypedElements(napi_env env, napi_value value, size_t& bytes);

constexpr auto NUM_1 { 1 };
constexpr auto NUM_2 { 2 };
constexpr auto NUM_3 { 3 };
constexpr auto NUM_8 { 8 };
constexpr auto NUM_16 { 16 };
constexpr auto NUM_24 { 24 };

template<>
struct InteropTypeConverter<KStringArray> {
    using InteropType = napi_value;
    using SelfType = InteropTypeConverter<KStringArray>;
    static constexpr auto LengthByteSize = sizeof(KUInt);
    ...
    {
        size_t bytes = 0;
        auto const encoded = getTypedElements<uint8_t>(env, value, bytes);
        if (!bytes)
            return { { nullptr, nullptr }, 0 };

        size_t const num = decodeLength(encoded);
        size_t bufferSize = sizeof(char*) * (num + 1);
        char** mem = static_cast<char**>(malloc(bufferSize));
        if (mem == nullptr) {
            INTEROP_FATAL("malloc failed, size = %zu", bufferSize);
            return { { nullptr, nullptr }, 0 };
        }
        KStringArray::Holder result(mem, &free);
...
template<typename ElemType>
ElemType* getTypedElementsImpl(napi_env env, napi_value value, size_t& bytesLength)
{
    ...
    napi_status status = napi_get_typedarray_info(env, value, &type, &bytesLength, &data, &arrayBuffer, &byteOffset);
...
{
    return getTypedElementsImpl<ElemType>(env, value, bytesLength);
}
...
{
    size_t dummy;
    return getTypedElements<ElemType>(env, value, dummy);
}
...
template<>
inline KStringArray getArgument<KStringArray>(const CallbackInfo& info, int index)
{
    NAPI_ASSERT_INDEX(info, index, {});
    return getArgument<KStringArray>((napi_env)info.Env(), (napi_value)info[index]);
}
