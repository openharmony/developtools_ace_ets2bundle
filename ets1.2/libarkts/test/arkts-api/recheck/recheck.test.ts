...
function dumpSrc(file: string) {
    const src = arkts.arktsGlobal.compilerContext!.program.ast.dumpSrc()
...
function dumpJson(file: string) {
    const json = arkts.arktsGlobal.compilerContext!.program.ast.dumpJson()
...
function assertSrc(file: string) {
    ...
    util.assert.equal(filterGensym(filterLicense(src)), filterGensym(filterLicense(expected)))
}
...
function filterLicense(src: string): string {
    if (src.startsWith('/*')) {
        let index = src.indexOf('*/')
        return src.substring(index + 3)
    }
    return src
}
...
suite(util.basename(__filename), () => {
    ...
    suite('exports', () => {
        ...
        // See es2panda issue 31786
        test.skip('import created class', () => {
