...
## 源码参考位置
- 动态：`compiler/src/validate_ui_syntax.ts`（`validateSingleDecorator`）
- 静态：`arkui-plugins/collectors/ui-collectors/validators/rules/check-computed-decorator.ts`（`checkComputedDecorator`）
...
## 静态
### 源码参考位置
- `arkui-plugins/collectors/ui-collectors/validators/rules/check-computed-decorator.ts`（`checkComputedDecorator`）
### 静态工具链处理
...
- `compiler/src/validate_ui_syntax.ts`（`validateClassDecorator`）

## 动静态差异说明

| 维度 | 动态工具链 | 静态工具链 |
|---|---|---|
| 校验入口 | `validate_ui_syntax.ts`（`validateSingleDecorator`） | `check-computed-decorator.ts`（`checkComputedDecorator`） |
