...
## 动态
### 源码参考位置
- `compiler/src/process_visual.ts`（`visualTransform`，Rollup 插件入口）
- `compiler/src/process_visual.ts`（`parseVisual`，含 `sourceReplace` + `validateUISyntax`）
- `compiler/src/process_visual.ts`（`parseStatement`，遍历 struct 声明）
- `compiler/src/process_visual.ts`（`parseMember`，检测空 build 方法）
- `compiler/src/process_visual.ts`（`insertVisualCode`，四步注入主控）
- `compiler/src/process_visual.ts`（`insertImport`，注入导入语句）
- `compiler/src/process_visual.ts`（`insertVarAndFunc`，注入变量和函数）
- `compiler/src/process_visual.ts`（`insertBuild`，注入 build 方法内容）
- `compiler/src/process_visual.ts`（`insertAboutToAppear`，注入/追加 aboutToAppear）
- `compiler/src/process_visual.ts`（`insertVisualCodeAfterPos`，位置后插入）
- `compiler/src/process_visual.ts`（`insertVisualCodeBeforePos`，位置前插入）
- `compiler/src/process_visual.ts`（`generateSourceMapForNewAndOriEtsFile`，sourcemap 生成）
- `compiler/src/process_visual.ts`（`findVisualFile`，查找 .visual 文件路径）
- `compiler/src/process_visual.ts`（`getVisualContent`，解析 .visual 文件）
- `compiler/src/process_visual.ts`（`getParsedContent`，主入口）
- `compiler/src/process_visual.ts`（`clearVisualSlotMap`，清理映射）
- `compiler/src/pre_define.ts`（`MODULE_VISUAL_PATH = 'src/main/supervisual'`）
- `compiler/src/pre_define.ts`（`SUPERVISUAL = './supervisual'`）
- `compiler/src/pre_define.ts`（`SUPERVISUAL_SOURCEMAP_EXT = '.visual.js.map'`）
