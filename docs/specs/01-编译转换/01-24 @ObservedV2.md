...
- `compiler/src/validate_ui_syntax.ts`（`checkObservedProperty`）
- `compiler/src/pre_define.ts`（`COMPONENT_DECORATOR_REUSEABLE`，附近 `COMPONENT_OBSERVEDV2_DECORATOR = '@ObservedV2'`）
...
### 关键转换逻辑
- `isObservedClass`：检测类是否有 `@ObservedV2` 装饰器。
- `checkObservedProperty`：校验 `@ObservedV2` 类的属性规则。
...
## 静态
### 源码参考位置
- `arkui-plugins/common/predefines.ts`（`ObservedNames` 枚举）
...
- `compiler/src/validate_ui_syntax.ts`（`validateInheritClassDecorator`）
- `compiler/src/validate_ui_syntax.ts`（`validateMutilObserved`）
...
| 常量 | `declare class ObservedV2` | `COMPONENT_OBSERVEDV2_DECORATOR = '@ObservedV2'`（`pre_define.ts`） | `DecoratorNames.OBSERVED_V2 = 'ObservedV2'` |
...
| 维度 | SDK 声明 | 动态工具链 | 静态工具链 |
|---|---|---|---|
| 常量 | `declare class Trace` | `CLASS_MIN_TRACK_DECORATOR = 'Trace'`（`pre_define.ts`） | `DecoratorNames.TRACE = 'Trace'` |
| 使用范围 | `@ObservedV2` class 的属性 | 同 | 同 |
| 验证 | - | `process_component_member.ts`：仅限 `@ObservedV2` 或 `makeV1Observed` | property-translators 处理 |

**验证逻辑**（动态工具链 `process_component_member.ts`）：
```
