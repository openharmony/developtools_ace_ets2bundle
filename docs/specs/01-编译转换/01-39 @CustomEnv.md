...
- `compiler/src/validate_ui_syntax.ts`（校验逻辑）
- `compiler/src/pre_define.ts`（`COMPONENT_CUSTOM_ENV_DECORATOR = '@CustomEnv'`）
...
## 静态
### 源码参考位置
- `arkui-plugins/ui-plugins/property-translators/customEnv.ts`（`getCustomEnvKey`）
- `arkui-plugins/ui-plugins/property-translators/customEnv.ts`（`initializeStructWithCustomEnvProperty`）
- `arkui-plugins/ui-plugins/property-translators/customEnv.ts`（`CustomEnvCachedTranslator`）
...
- `getCustomEnvKey`：从注解中提取 `value` 属性作为 env key。
