...
## 动态
### 源码参考位置
- `compiler/src/process_component_member.ts`（`addAddProvidedVar`）
- `compiler/src/process_component_member.ts`（`createAddProvidedVar`）
- `compiler/src/pre_define.ts`（`COMPONENT_PROVIDE_DECORATOR = '@Provide'`）
...
## 静态
### 源码参考位置
- `arkui-plugins/ui-plugins/property-translators/provide.ts`（`initializeStructWithProvideProperty`）
- `arkui-plugins/common/predefines.ts`（`DecoratorNames.PROVIDE = 'Provide'`）
- `arkui-plugins/common/predefines.ts`（`StateManagementTypes.PROVIDE_DECORATED`）
- `arkui-plugins/common/predefines.ts`（`StateManagementTypes.MAKE_PROVIDE = 'makeProvide'`）
...
- @Provide 的 alias 参数：`addAddProvidedVar`（`process_component_member.ts`）生成 `this.addProvidedVar('name', this.__prop)`
...
- alias 和 allowOverride 参数提取：`process_component_member.ts`（`addAddProvidedVar`）
- @Provide 初始化与 @State 相同，额外注册到 ProvidedVar
- @Provide 的 `allowOverride` 参数：`process_component_member.ts`（`addAddProvidedVar` 中的 allowOverride 逻辑）
