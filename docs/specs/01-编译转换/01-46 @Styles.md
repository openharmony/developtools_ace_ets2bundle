...
## 动态
### 源码参考位置
- `compiler/src/component_map.ts`（`INNER_STYLE_FUNCTION`/`GLOBAL_STYLE_FUNCTION`）
- `compiler/src/validate_ui_syntax.ts`（`collectStyles`，全局收集）
- `compiler/src/process_component_class.ts`（组件内 `@Styles` 收集）
- `compiler/src/process_component_build.ts`（`traverseStateStylesAttr`，stateStyles 中引用展开）
- `compiler/src/pre_define.ts`（`COMPONENT_STYLES_DECORATOR = '@Styles'`）
...
> 深度逻辑：`@Styles` 在 `stateStyles` 中引用展开（`traverseStateStylesAttr`），只展开 body 第 1 条语句。

## 静态
### 源码参考位置
- `arkui-plugins/common/predefines.ts`（`DecoratorNames.BUILDER`，附近 enum）
- 静态工具链通过样式收集与展开机制处理
...
### 源码参考位置
- `compiler/src/validate_ui_syntax.ts`（`collectStyles`，全局 @Styles 收集到 `GLOBAL_STYLE_FUNCTION`）
- `compiler/src/process_component_class.ts`（组件内 @Styles 收集到 `INNER_STYLE_FUNCTION`）
- `compiler/src/pre_define.ts`（`COMPONENT_STYLES_DECORATOR = '@Styles'`）
- `compiler/src/process_component_build.ts`（@Styles 直接作为属性调用的展开）
- `compiler/src/process_component_build.ts`（`traverseStateStylesAttr`，stateStyles 中的 @Styles 引用展开）

### 收集阶段

#### 全局 @Styles（`collectStyles`，validate_ui_syntax.ts）
...
#### 组件内 @Styles（process_component_class.ts）
...
- 限制：`@Styles` 方法不能有参数，否则报错 `10905105`
...
- 如果是 RecycleComponent，同时展开到 `updateStatements`

#### 2. @Styles 在 stateStyles 中的引用展开

`traverseStateStylesAttr` 遍历 `stateStyles` 参数对象的属性，逆序处理（`reverse()`）：
...
2. **参数限制**：`@Styles` 方法和函数不能有参数，否则报错 `10905105`。
