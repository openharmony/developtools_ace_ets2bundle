...
- `compiler/src/pre_define.ts`（`WRAPPEDBUILDER_CLASS = 'WrappedBuilder'`）
- `compiler/src/pre_define.ts`（`WRAPBUILDER_BUILDERPROP = 'builder'`）
- `compiler/src/pre_define.ts`（`MUTABLEBUILDER_FUNCTION = 'mutableBuilder'`）
- `compiler/src/pre_define.ts`（`MUTABLEBUILDER_CLASS = 'MutableBuilder'`）
- `compiler/src/process_ui_syntax.ts`（`isWrapBuilderFunction`）
- `compiler/src/process_ui_syntax.ts`（`isMutableBuilderFunction`）
- `compiler/src/process_ui_syntax.ts`（处理分支，校验参数）
...
### 关键转换逻辑
1. **检测**（`isWrapBuilderFunction`）：判断节点是否为 `CallExpression`，且 `node.expression` 为 Identifier 且 `escapedText === 'wrapBuilder'`。
2. **参数校验**：
   ```
...
3. **mutableBuilder 处理**（`isMutableBuilderFunction`）：同理检测 `mutableBuilder` 调用，参数校验逻辑一致。
