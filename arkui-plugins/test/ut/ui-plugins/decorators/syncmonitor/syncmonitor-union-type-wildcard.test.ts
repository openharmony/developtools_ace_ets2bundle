...
buildConfig.compileFiles = [
    path.resolve(getRootPath(), MOCK_ENTRY_DIR_PATH, STATE_DIR_PATH, 'syncmonitor-union-type-wildcard.ets'),
];

const projectConfig: ProjectConfig = mockProjectConfig();
projectConfig.compatibleSdkVersion = 26;

const pluginTester = new PluginTester('test @SyncMonitor decorator with union types and wildcard', buildConfig, projectConfig);
...
import { SyncMonitor as SyncMonitor, Local as Local, IMonitor as IMonitor, ObservedV2 as ObservedV2, Trace as Trace } from "@ohos.arkui.stateManagement";
...
__EntryWrapper.RegisterNamedRouter("", new __EntryWrapper(), ({
  ...
  moduleName: "entry",
  pagePath: "../../../decorators/syncmonitor/syncmonitor-union-type-wildcard",
  pageFullPath: "test/demo/mock/decorators/syncmonitor/syncmonitor-union-type-wildcard",
  integratedHsp: "false",
} as NavInterface));
...
@Entry() @ComponentV2() final struct Index extends CustomComponentV2<Index, __Options_Index> implements PageLifeCycle {
  ...
  @SyncMonitor({value:["entity.*", "entity.name", "container.inner.*"]}) 
...
pluginTester.run(
    'test @SyncMonitor decorator with union types and wildcard',
