...
buildConfig.compileFiles = [
    path.resolve(getRootPath(), MOCK_ENTRY_DIR_PATH, STATE_DIR_PATH, 'monitor-union-type.ets'),
];

const projectConfig: ProjectConfig = mockProjectConfig();
projectConfig.compatibleSdkVersion = 26;

const pluginTester = new PluginTester('test @Monitor decorator with union types', buildConfig, projectConfig);
...
@ComponentV2() final struct Index extends CustomComponentV2<Index, __Options_Index> {
  public __initializeStruct(initializers: (__Options_Index | undefined), @Memo() content: ((()=> void) | undefined)): void {
    ...
    this.__backing_items2 = STATE_MGMT_FACTORY.makeLocal<ObservedArray<(Person | Animal)>>(this, "items2", new ObservedArray<(Person | Animal)>());
    this.__backing_items3 = STATE_MGMT_FACTORY.makeLocal<(Map<string, (Person | Animal)> | string | number)>(this, "items3", new Map<string, (Person | Animal)>());
    this.__backing_items4 = STATE_MGMT_FACTORY.makeLocal<(Set<(Person | Animal)> | string | number)>(this, "items4", new Set<(Person | Animal)>());
    this.__monitor_onEntityChange = STATE_MGMT_FACTORY.makeMonitor([({
      ...
      path: "items.length",
      valueCallback: ((): Any => {
        return ({let gensym___<some_random_number> = this.items;
          (((gensym___<some_random_number>) == (null)) ? undefined : gensym___<some_random_number>.length)});
      }),
    } as IMonitorPathInfo), ({
      path: "items2.0.species",
      valueCallback: ((): Any => {
        let x: Any = this.items2;
        if (((x) instanceof (ObservedArray))) {
          x = x.$_get(0);
        } else {
          return undefined;
        }
      ...
      path: "items2.length",
      valueCallback: ((): Any => {
        return ({let gensym___<some_random_number> = this.items2;
          (((gensym___<some_random_number>) == (null)) ? undefined : gensym___<some_random_number>.length)});
      }),
    } as IMonitorPathInfo), ({
      path: "items3.size",
      valueCallback: ((): Any => {
        let x: Any = this.items3;
        if (((x) instanceof (Map))) {
          x = x.size;
        } else {
          return undefined;
  ...
  public resetStateVarsOnReuse(initializers: (__Options_Index | undefined)): void {
    ...
    this.__backing_items2!.resetOnReuse(new ObservedArray<(Person | Animal)>());
    this.__backing_items3!.resetOnReuse(new Map<string, (Person | Animal)>());
    this.__backing_items4!.resetOnReuse(new Set<(Person | Animal)>());
  ...
  private __backing_items2?: ILocalDecoratedVariable<ObservedArray<(Person | Animal)>>;
  public get items2(): ObservedArray<(Person | Animal)> {
    return this.__backing_items2!.get();
  }
  public set items2(value: ObservedArray<(Person | Animal)>) {
    this.__backing_items2!.set(value);
  }
  private __backing_items3?: ILocalDecoratedVariable<(Map<string, (Person | Animal)> | string | number)>;
  public get items3(): (Map<string, (Person | Animal)> | string | number) {
    return this.__backing_items3!.get();
  }
  public set items3(value: (Map<string, (Person | Animal)> | string | number)) {
    this.__backing_items3!.set(value);
  }
  private __backing_items4?: ILocalDecoratedVariable<(Set<(Person | Animal)> | string | number)>;
  public get items4(): (Set<(Person | Animal)> | string | number) {
    return this.__backing_items4!.get();
  }
  public set items4(value: (Set<(Person | Animal)> | string | number)) {
    this.__backing_items4!.set(value);
  }
  
  private __monitor_onEntityChange: (IMonitorDecoratedVariable | undefined);

  @Monitor({value:["entity.name", "items.0.species", "items.length", "items2.0.species", "items2.length", "items3.size", "items4.size"]}) 
...
@ComponentV2() class __Options_Index {
  ...
  @Local() public items2?: ObservedArray<(Person | Animal)>;
  public __backing_items2?: ILocalDecoratedVariable<ObservedArray<(Person | Animal)>>;
  public __options_has_items2?: boolean;
  @Local() public items3?: (Map<string, (Person | Animal)> | string | number);
  public __backing_items3?: ILocalDecoratedVariable<(Map<string, (Person | Animal)> | string | number)>;
  public __options_has_items3?: boolean;
  @Local() public items4?: (Set<(Person | Animal)> | string | number);
  public __backing_items4?: ILocalDecoratedVariable<(Set<(Person | Animal)> | string | number)>;
  public __options_has_items4?: boolean;
  public constructor() {}
...
pluginTester.run(
    'test @Monitor decorator with union types',
