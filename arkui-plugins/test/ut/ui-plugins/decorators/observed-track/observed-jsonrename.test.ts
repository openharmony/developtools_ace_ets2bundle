...
buildConfig.compileFiles = [
    path.resolve(getRootPath(), MOCK_ENTRY_DIR_PATH, OBSERVED_DIR_PATH, 'observed-jsonrename.ets'),
];

const observedJsonRenameTransform: Plugins = {
    name: 'observedJsonRename',
    parsed: uiTransform().parsed,
}

const pluginTester = new PluginTester('test observed class transform with JsonRename annotation', buildConfig);
...
function testObservedJsonRenameTransformer(this: PluginTestContext): void {
    expect(parseDumpSrc(this.scriptSnapshot ?? '')).toBe(parseDumpSrc(expectedScript));
}

pluginTester.run(
    'test observed transform with JsonRename annotation',
