...
const NO_DUPLICATE_ENTRY_DIR_PATH = "unit/no_duplicate_entry";
const NO_DUPLICATE_ENTRY_ETS_NAME = "no_duplicate_entry_2";

const testJson = new TestJsonPath(
    NO_DUPLICATE_ENTRY_DIR_PATH,
);

const compileFile = new CompileFilePath(
    NO_DUPLICATE_ENTRY_DIR_PATH,
    NO_DUPLICATE_ENTRY_ETS_NAME
);
...
async function testParsedTransformer(this: PluginTestContext): Promise<void> {
    ...
    const currentTest = testData[NO_DUPLICATE_ENTRY_ETS_NAME].messages;
    const missing = unitTestParsedTransformer(this, currentTest, NO_DUPLICATE_ENTRY_ETS_NAME);
    expect(missing).toEqual([]);
}

buildConfig.compileFiles = [compileFile.value];
const pluginTester = new PluginTester("No Duplicate Entry", buildConfig);
