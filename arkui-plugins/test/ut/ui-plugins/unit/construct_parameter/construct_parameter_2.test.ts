...
const CONSTRUCT_PAR_DIR_PATH = "unit/construct_parameter";
const CONSTRUCT_PAR_ETS_NAME = "construct_parameter_2";

const testJson = new TestJsonPath(
    CONSTRUCT_PAR_DIR_PATH,
);

const compileFile = new CompileFilePath(
    CONSTRUCT_PAR_DIR_PATH,
    CONSTRUCT_PAR_ETS_NAME
);
...
function testParsedTransformer(this: PluginTestContext): void {
    ...
    const currentTest = testData[CONSTRUCT_PAR_ETS_NAME].messages;
    const missing = unitTestParsedTransformer(this, currentTest, CONSTRUCT_PAR_ETS_NAME);
