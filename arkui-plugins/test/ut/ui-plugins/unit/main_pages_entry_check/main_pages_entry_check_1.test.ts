...
const MAIN_PAGES_ENTRY_DIR_PATH = "unit/main_pages_entry_check";
const MAIN_PAGES_ENTRY_ETS_NAME = "main_pages_entry_check_1";

const testJson = new TestJsonPath(
    MAIN_PAGES_ENTRY_DIR_PATH,
);

const compileFile = new CompileFilePath(
    MAIN_PAGES_ENTRY_DIR_PATH,
    MAIN_PAGES_ENTRY_ETS_NAME
);

const mainPagesTransform: Plugins = {
...
async function testParsedTransformer(this: PluginTestContext): Promise<void> {
    ...
    const currentTest = testData[MAIN_PAGES_ENTRY_ETS_NAME].messages;
    const missing = unitTestParsedTransformer(this, currentTest, MAIN_PAGES_ENTRY_ETS_NAME);
    expect(missing).toEqual([]);
}

buildConfig.compileFiles = [compileFile.value];
const pluginTester = new PluginTester("Main Pages Entry Check", buildConfig);
