...
const ATTR_NO_INVOKE_DIR_PATH = "unit/attribute_no_invoke";
const ATTR_NO_INVOKE_ETS_NAME = "attribute_no_invoke_6";

const testJson = new TestJsonPath(
    ATTR_NO_INVOKE_DIR_PATH
);

const compileFile = new CompileFilePath(
    ATTR_NO_INVOKE_DIR_PATH,
    ATTR_NO_INVOKE_ETS_NAME
);

const attributeInvokeTransform: Plugins = {
...
async function testParsedTransformer(this: PluginTestContext): Promise<void> {
    ...
    const currentTest = testData[ATTR_NO_INVOKE_ETS_NAME].messages;
    const missing = unitTestParsedTransformer(this, currentTest, ATTR_NO_INVOKE_ETS_NAME);
    expect(missing).toEqual([]);
}

buildConfig.compileFiles = [compileFile.value];
const pluginTester = new PluginTester("Attribute No Invoke Check", buildConfig);

pluginTester.run(
    ATTR_NO_INVOKE_ETS_NAME,
    [attributeInvokeTransform],
