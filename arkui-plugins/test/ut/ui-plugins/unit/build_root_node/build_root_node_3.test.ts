...
const BUILD_ROOT_NODE_DIR_PATH = "unit/build_root_node";
const BUILD_ROOT_NODE_ETS_NAME = "build_root_node_3";

const testJson = new TestJsonPath(
    BUILD_ROOT_NODE_DIR_PATH
);

const compileFile = new CompileFilePath(
    BUILD_ROOT_NODE_DIR_PATH,
    BUILD_ROOT_NODE_ETS_NAME
);

const buildRootNodeTransform: Plugins = {
...
async function testParsedTransformer(this: PluginTestContext): Promise<void> {
    ...
    const currentTest = testData[BUILD_ROOT_NODE_ETS_NAME].messages;
    const missing = unitTestParsedTransformer(this, currentTest, BUILD_ROOT_NODE_ETS_NAME);
    expect(missing).toEqual([]);
}

buildConfig.compileFiles = [compileFile.value];
const pluginTester = new PluginTester("Build Root Node Check", buildConfig);

pluginTester.run(
    BUILD_ROOT_NODE_ETS_NAME,
    [buildRootNodeTransform],
