...
const COMPONENT_V2_MIX_USE_DIR_PATH = "unit/component_componentV2_mix_use_check";
const COMPONENT_V2_MIX_USE_ETS_NAME = "component_componentV2_mix_use_check_6";

const testJson = new TestJsonPath(
    COMPONENT_V2_MIX_USE_DIR_PATH,
);

const compileFile = new CompileFilePath(
    COMPONENT_V2_MIX_USE_DIR_PATH,
    COMPONENT_V2_MIX_USE_ETS_NAME
);
...
function testParsedTransformer(this: PluginTestContext): void {
    ...
    const currentTest = testData[COMPONENT_V2_MIX_USE_ETS_NAME].messages;
    const missing = unitTestParsedTransformer(this, currentTest, COMPONENT_V2_MIX_USE_ETS_NAME);
    expect(missing).toEqual([]);
}

buildConfig.compileFiles = [compileFile.value];
const pluginTester = new PluginTester("ComponentV2 Mix Use Check", buildConfig);
