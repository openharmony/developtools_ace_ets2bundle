...
const PROPERTY_TYPE_DIR_PATH = "unit/property_type";
const PROPERTY_TYPE_ETS_NAME = "property_type_1";

const testJson = new TestJsonPath(
  PROPERTY_TYPE_DIR_PATH,
);

const compileFile = new CompileFilePath(
  PROPERTY_TYPE_DIR_PATH,
  PROPERTY_TYPE_ETS_NAME
);

const propertyTypeTransform: Plugins = {
...
async function testParsedTransformer(this: PluginTestContext): Promise<void> {
  ...
  const currentTest = testData[PROPERTY_TYPE_ETS_NAME].messages;
  const missing = unitTestParsedTransformer(this, currentTest, PROPERTY_TYPE_ETS_NAME);
  expect(missing).toEqual([]);
}

buildConfig.compileFiles = [compileFile.value];

const pluginTester = new PluginTester("property type test", buildConfig);
