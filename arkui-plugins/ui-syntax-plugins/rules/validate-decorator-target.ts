...
// Can only be used with decorators for proerty
const proertyOnlyDecorators = [
...
function validateDecoraterStructOnly(node: arkts.AstNode, context: UISyntaxRuleContext): void {
  // class
  if (arkts.isClassDeclaration(node)) {
    node.definition?.annotations?.forEach((annotation) => {
      validateDecorator(annotation, structOnlyDecorators, rule.messages.decoratorOnlyWithStruct, context);
    });
  }
...
function validateDecoratorOnMethod(
  node: arkts.MethodDefinition,
  context: UISyntaxRuleContext,
): void {
  if (arkts.isScriptFunction(node.scriptFunction)) {
    node.scriptFunction.annotations?.forEach((annotation) => {
      validateDecorator(annotation, proertyOnlyDecorators, rule.messages.decoratorOnlyWithMemberProerty, context);
    });
  }
}

const rule: UISyntaxRule = {
  name: 'validate-decorator-target',
  messages: {
    decoratorOnlyWithStruct: `The '@{{decoratorName}}' decorator can only be used with 'struct'.`,
    decoratorOnlyWithMemberProerty: `'@{{decoratorName}}' can only decorate member property.`
  },
  setup(context) {
    return {
      parsed: (node): void => {
        if (arkts.isMethodDefinition(node)) {
          validateDecoratorOnMethod(node, context);
        }
        if (!arkts.isStructDeclaration(node)) {
          validateDecoraterStructOnly(node, context);
        }
      },
    };
  },
