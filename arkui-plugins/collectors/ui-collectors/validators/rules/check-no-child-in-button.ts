...
function _checkNoChildInButton(this: BaseValidator<arkts.CallExpression, CallInfo>, node: arkts.CallExpression): void {
    ...
    const hasLabel = node.arguments.length > 0 && arkts.isStringLiteral(node.arguments[0]);
    const trailingClosure = node.arguments.find((arg) => arkts.isArrowFunctionExpression(arg));
    if (hasLabel && trailingClosure) {
        this.report({
            node: node,
            level: LogType.ERROR,
            message: `The Button component with a label parameter can not have any child.`,
            suggestions: [createSuggestion('', ...getPositionRangeFromNode(trailingClosure), `Remove child components`)],
        });
    }
}
