...
import { MemoPluginContext } from "../common"
...
export abstract class ScopedVisitor extends arkts.AbstractVisitor {
    ...
    protected constructor(
        protected memoPluginContext: MemoPluginContext,
    ) {
        super()
    }
    ...
    protected enterScope(node: arkts.AstNode): void {
        if (arkts.isScriptFunction(node)) {
            ...
            const isMemo = !!this.memoPluginContext.ScriptFunctionDescriptors?.get(node.peer)?.kind
