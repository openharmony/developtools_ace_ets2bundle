...
export function getNormalizedOhmUrlByModuleRequest(moduleInfoByModuleRequets: Object, projectConfig: Object,
  logger?: Object): string {
  const normalizedPath = toUnixPath(moduleInfoByModuleRequets.normalizedPath);
  const pkgName = moduleInfoByModuleRequets.packageName;
...
export function getOhmUrlByByteCodeHar(moduleRequest: string, projectConfig: Object, rollupObject: Object, logger?: Object):
  ...
  if (projectConfig.byteCodeHarInfo) {
    const moduleInfoByModuleRequets: Object = rollupObject.share?.importResolver(moduleRequest);
    if (moduleInfoByModuleRequets) {
       return getNormalizedOhmUrlByModuleRequest(moduleInfoByModuleRequets, projectConfig, logger);
    }
...
export function getOhmUrlByExternalPackage(moduleRequest: string, projectConfig: Object, rollupObject: Object, logger?: Object,
  ...
  if (Object.keys(externalPkgMap).length !== 0) {
    const moduleInfoByModuleRequets: Object = rollupObject.share?.importResolver(moduleRequest);
    if (useNormalizedOHMUrl && moduleInfoByModuleRequets) {
       return getNormalizedOhmUrlByModuleRequest(moduleInfoByModuleRequets, projectConfig, logger);
    }
