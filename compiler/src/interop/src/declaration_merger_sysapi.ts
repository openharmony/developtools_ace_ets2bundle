...
export function findSystemApiModuleName(
  ...
  const visited: Set<ts.Symbol> = new Set();
  const pendingModuleSymbols: ts.Symbol[] = [];
  let current: ts.Symbol | undefined = symbol;
  while (current && !visited.has(current)) {
    ...
    for (const decl of current.declarations ?? []) {
      ...
      const moduleSym: ts.Symbol | undefined = findModuleSymbolOfDeclaration(ctx.checker, decl);
      if (moduleSym) {
        pendingModuleSymbols.push(moduleSym);
      }
    }
  ...
  return searchSystemApiInPendingModules(ctx, pendingModuleSymbols, symbol, current);
}

function findModuleSymbolOfDeclaration(
  checker: ts.TypeChecker, decl: ts.Declaration
): ts.Symbol | undefined {
  let current: ts.Node | undefined = decl;
  while (current) {
    if (ts.isImportDeclaration(current) || ts.isExportDeclaration(current)) {
...
function searchSystemApiInPendingModules(
  ctx: SysApiContext,
  pendingModuleSymbols: ts.Symbol[],
  originSymbol: ts.Symbol,
  targetSymbol: ts.Symbol | undefined
): { moduleName: string; statementText: string } | null {
  if (pendingModuleSymbols.length === 0) {
    return null;
  }
  const target: ts.Symbol = targetSymbol ?? originSymbol;
  const name: string = originSymbol.getName();
  for (const moduleSym of pendingModuleSymbols) {
    for (const decl of moduleSym.declarations ?? []) {
      if (!ts.isSourceFile(decl)) {
        continue;
      }
      const found = searchImportInSourceFile(ctx, decl, name, target);
      if (found && specifierMatchesSystemModule(ctx.systemModules, found.moduleName)) {
        return found;
      }
    }
  }
