...
function validateObservedProperty(node: ts.PropertyDeclaration, isComponentV2: boolean, log: LogInfo[],
  ...
  const symbol = getSymbolIfAliased(node.initializer.expression);
  ...
  if (languageVersion?.languageVersion === "1.2") {
    const [hasObserved, hasObservedV2] = checkObservedProperty(node);
    if ((isComponentV2 && hasObserved) || (!isComponentV2 && hasObservedV2)) {
      ...
      addLog(LogType.ERROR, message, node.getStart(), log, sourceFileNode, { code: '10905502' });
      return true;
    }
  }
  return false;
...
function validatePropertyInStruct(structContext: boolean, decoratorNode: ts.Identifier,
  ...
  let message: string;
  if (ts.isNewExpression(propertyNode.initializer) && ts.isIdentifier(propertyNode.initializer.expression)) {
    const symbol: ts.Symbol = getSymbolIfAliased(propertyNode.initializer.expression);
    if (!symbol?.valueDeclaration) {
      return;
    }
    const filePath = symbol.valueDeclaration.getSourceFile().fileName;
    ...
    if (languageVersion?.languageVersion === "1.2") {
      if ((isV1Decorator && classResult.hasObservedV2) || (isV2Decorator && classResult.hasObserved)) {
        ...
        message = `The type of the @${decoratorName} property can not be a class decorated with` +
