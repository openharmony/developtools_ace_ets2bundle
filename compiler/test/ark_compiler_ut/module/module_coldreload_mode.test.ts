...
import { toUnixPath, mkdirsSync } from '../../../lib/utils';
import {
  SOURCEMAPS,
  EXTNAME_TS,
  EXTNAME_ETS
...
mocha.describe('test module_coldreload_mode file api', function () {
  ...
  mocha.it('1-7: test compileContextInfo regenerated by changedFileList under cold reload not first build', function() {
    this.rollup.coldReload();
    this.rollup.useNormalizedOHMUrl();
    SourceMapGenerator.initInstance(this.rollup);
    this.rollup.mockCompileContextInfo();

    const changedFile: string = 'pages/ChangedIndex.ets';
    const changedFileInAbsolutePath: string =
      path.join(this.rollup.share.projectConfig.projectPath, changedFile);
    const changedFileListPath: string = path.join(this.rollup.share.projectConfig.cachePath, 'changedFileList.json');
    mkdirsSync(path.dirname(changedFileListPath));
    fs.writeFileSync(changedFileListPath, JSON.stringify({ 'modifiedFiles': [changedFile] }), 'utf-8');
    this.rollup.share.projectConfig.changedFileList = changedFileListPath;

    const moduleMode = new ModuleColdreloadMode(this.rollup);
    expect(moduleMode.compileContextInfoPath !== undefined).to.be.true;
    const originalCompileContextInfo: string = fs.readFileSync(moduleMode.compileContextInfoPath, 'utf-8');
    this.rollup.mockCompileContextInfo({ 'page/ChangedIndex': changedFileInAbsolutePath }, 'entry');

    const stubAbc = sinon.stub(moduleMode, 'generateAbcByEs2abc');
    const stubSourceMap = sinon.stub(moduleMode, 'updateSourceMapFromFileList');
    try {
      moduleMode.compileChangeListFiles(this.rollup, undefined);
      const entryObj: Array<string> = moduleMode.projectConfig.entryObj;
      expect(Array.isArray(entryObj)).to.be.true;
      expect(entryObj[0] === changedFileInAbsolutePath).to.be.true;
      const newCompileContextInfo: string = fs.readFileSync(moduleMode.compileContextInfoPath, 'utf-8');
      expect(newCompileContextInfo !== originalCompileContextInfo).to.be.true;
      const compileContextInfo: Object = JSON.parse(newCompileContextInfo);
      expect(compileContextInfo.compileEntries.length > 0).to.be.true;
    } finally {
      stubAbc.restore();
      stubSourceMap.restore();
      SourceMapGenerator.cleanSourceMapObject();
    }
  });
});
